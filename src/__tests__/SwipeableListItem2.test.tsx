...
import { ActionAnimations, SwipeableListItem2 as SwipeableListItem } from "../SwipeableListItem";
import {
    swipeLeftMouse,
    swipeLeftTouch,
    swipeRightMouse,
...
test("left swipe action triggering", () => {
    const callbackLeft = vi.fn();
    const callbackRight = vi.fn();
...
test("start and end callbacks not triggered if swipe content not defined", () => {
    const callbackSwipeStart = vi.fn();
    const callbackSwipeEnd = vi.fn();
